...
static void moveX(int x)
{
  ...
  m = (struct MotionStats*)tv_EntityGetComponent(possessed, COMPONENT_MOTIONSTATS);
  if(m == NULL)
    return;
...
static void globalUpdate()
{
  ...
  if(possessed == NULL)
    return;
  t = (struct Transform*)tv_EntityGetComponent(possessed, COMPONENT_TRANSFORM);
  if(t != NULL){
    t->rot.y = rot;
...
void FpsControllerPossess(struct tv_Entity *e)
{
  possessed = e;
}
